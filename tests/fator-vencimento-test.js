...
// primeiroCiclo tests
test('primeiroCiclo - deve calcular fator correto para início do período', (t) => {
...
test('primeiroCiclo - deve calcular fator correto para final do primeiro ciclo', (t) => {
  const boleto = Boleto.novoBoleto();
...
// segundoCiclo tests
test('segundoCiclo - deve reiniciar contador no início do segundo ciclo', (t) => {
  const boleto = Boleto.novoBoleto();
  ...
  t.is(fator, '1000'); // FEBRABAN FB-009/2023: reinicia em 1000
});

test('segundoCiclo - deve calcular fator correto para data no meio do segundo ciclo', (t) => {
  const boleto = Boleto.novoBoleto();
  ...
  t.regex(fator, /^\d{4}$/);
  t.true(parseInt(fator) >= 0 && parseInt(fator) <= 9999);
});

test('segundoCiclo - deve calcular fator correto para final do segundo ciclo', (t) => {
  const boleto = Boleto.novoBoleto();
...
test('segundoCiclo - deve calcular fator correto para início do segundo ciclo + alguns dias', (t) => {
  const boleto = Boleto.novoBoleto();
  ...
  t.is(fator, '1001'); // 22/02 = 1000, 23/02 = 1001
});

test('segundoCiclo - deve calcular fator correto para data específica (13/10/2049)', (t) => {
  const boleto = Boleto.novoBoleto();
  const datas = Datas.novasDatas().comVencimento(new Date(2049, 9, 13)); // 13/10/2049
  ...
  t.is(fator, '9999'); // Final do segundo ciclo
});

// validacoesEdgeCases tests
test('validacoesEdgeCases - deve rejeitar data anterior à data base', (t) => {
  const boleto = Boleto.novoBoleto();
  ...
  t.throws(
    ...
    { message: /Data de vencimento não pode ser anterior/ }
  );
});

test('validacoesEdgeCases - deve rejeitar data após limite do segundo ciclo', (t) => {
  const boleto = Boleto.novoBoleto();
  ...
  t.throws(
    ...
    { message: /Aguardando nova especificação FEBRABAN/ }
  );
});

test('validacoesEdgeCases - deve rejeitar data inválida', (t) => {
  const boleto = Boleto.novoBoleto();
...
test('validacoesEdgeCases - deve sempre retornar string com 4 dígitos', (t) => {
  const boleto = Boleto.novoBoleto();
...
// casosEspecificos tests
test('casosEspecificos - deve demonstrar a transição entre ciclos', (t) => {
  ...
  t.is(boletoFimCiclo1.getFatorVencimento(), '9999');
  t.is(boletoInicioCiclo2.getFatorVencimento(), '1000'); // FEBRABAN FB-009/2023
});

// performance tests
test('performance - deve calcular fator rapidamente', (t) => {
  ...
  t.true(milliseconds < 100, `Performance inadequada: ${milliseconds}ms para 1000 cálculos`);
});
