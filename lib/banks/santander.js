...
class Santander {
  ...
  geraCodigoDeBarrasPara(boleto) {
    ...
    if (!carteirasValidas.includes(carteira)) {
      throw new Error(
        `Santander: Carteira inválida. Carteiras aceitas: ${carteirasValidas.join(', ')}. ` +
        `Recebido: ${carteira}`
      );
    }
    ...
    if (!nossoNumero || nossoNumero.length > 12) {
      throw new Error(
        `Santander: Nosso número deve ter até 12 dígitos. ` +
    ...
    // Fix: Usar código formatado (com padding) antes de fazer substring
    ...
    campoLivre.push('9');
    campoLivre.push(codigoFormatado.substring(0, 4));  // 4 primeiros dígitos
    campoLivre.push(codigoFormatado.substring(4));     // 3 últimos dígitos (total 7)
    campoLivre.push(nossoNumeroFormatado.substring(0, 7)); // 7 primeiros
    campoLivre.push(nossoNumeroFormatado.substring(7));    // 5 últimos (total 12)
    campoLivre.push(beneficiario.getDigitoNossoNumero());
    campoLivre.push('0'); // IOS - Identificador de Operação no Sistema
