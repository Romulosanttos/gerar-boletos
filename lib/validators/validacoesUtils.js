const formatacoes = require('../formatters/formatacoesUtils');
...
function eTituloDeEleitor(tituloDeEleitor) {
  ...
  const primeiraBase = tituloDeEleitor.substring(0, 8);
  let estado = tituloDeEleitor.substring(8, 10);
  const segundaBase = tituloDeEleitor.substring(8, 10);
...
function eEan(ean) {
  ...
  let peso = 1;
  const digitoVerificador = ean.pop();
  const soma = ean.reduceRight(function (anterior, atual) {
    peso = 4 - peso;
    return anterior + atual * peso;
  }, 0);
  const digitoVerificadorCalculado = soma + ((10 - (soma % 10)) % 10) - soma;
...
function eRegistroNacional(rn, tipo) {
  ...
  if (typeof tipo === 'undefined') {
    ...
    const fn = module.exports['e' + StringUtils.capitalize(tipo)];
...
const regexParaMatrizEFilial = /^[0-9]{8}([0-9]{4})[0-9]{2}$/;
