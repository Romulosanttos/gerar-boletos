    ...
    for (let indice = 0; indice < boletos.length; indice++) {
      ...
      if (pixEmv) {
        try {
          ...
          const qrCodeX = args.ajusteX + margemDoSegundoBloco + larguraInstrucoes + 60;
          const qrCodeY = args.ajusteY + tituloDaSetimaLinha + 12;
          ...
          const linhaVerticalX = args.ajusteX + margemDoSegundoBloco + larguraInstrucoes - 40;
          pdf
            .undash() // Garantir linha contínua (não tracejada)
            .moveTo(linhaVerticalX, qrCodeY - 13)
            .lineTo(linhaVerticalX, qrCodeY + 103)
            .lineWidth(1)
            .stroke(args.corDoLayout);
          ...
          const imagemPixWidth = 100; // Mesmo tamanho do QR Code
          const imagemPixHeight = 100; // Mesmo tamanho do QR Code
          const imagemPixX = linhaVerticalX + 3; // Mais próximo da linha
          const imagemPixY = qrCodeY; // Alinhado verticalmente com QR Code

          try {
            pdf.image(caminhoImagemPix, imagemPixX, imagemPixY, {
              width: imagemPixWidth,
              height: imagemPixHeight,
            });
          } catch (error) {
          ...
          instrucoesPixArray.forEach((instrucao, indice) => {
            pdf
              ...
              .text(
                instrucao,
                linhaVerticalX + 20, // Alinhado com início da área PIX
