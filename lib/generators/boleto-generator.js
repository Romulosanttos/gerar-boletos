...
const GeradorDeBoleto = (function () {
  ...
  GeradorDeBoleto.prototype.gerarPDF = async function (args) {
    ...
    for (let indice = 0; indice < boletos.length; indice++) {
      ...
      const larguraInstrucoes = pixEmv ? 280 : 400;
      ...
      if (pixEmv) {
        try {
          // Obter string EMV e instruções
          const emvString = boleto.getPixEmvString();
          const instrucoesPixArray = boleto.getPixInstrucoes();
          ...
          const qrCodeDataURL = await gerarQRCodePixDataURL(emvString, {
            width: 100, // Reduzido 15%
            margin: 1,
          });

          // Posicionar QR Code centralizado verticalmente na área de instruções
          // Posição ajustada para ficar à direita das instruções, bem centralizado
          const qrCodeX = args.ajusteX + margemDoSegundoBloco + larguraInstrucoes + 20;
          const qrCodeY = args.ajusteY + tituloDaSetimaLinha + 10; // Subido mais para melhor posicionamento

          pdf.image(qrCodeDataURL, qrCodeX, qrCodeY, {
            width: 100,
            height: 100,
          });

          // Instruções do PIX (cada linha é renderizada separadamente)
          // Posicionado à esquerda, alinhado com a base do QR Code
          const qrCodeHeight = 200;
          const espacamentoEntreLinhas = 10; // Espaçamento entre linhas de instrução
          const margemSuperiorTexto = 70; // Margem do texto em relação ao QR Code

          // Calcular posição inicial para centralizar verticalmente as instruções com o QR Code
          const totalHeightInstrucoes = instrucoesPixArray.length * espacamentoEntreLinhas;
          const textoPixY =
            qrCodeY + (qrCodeHeight - totalHeightInstrucoes) / 2 + margemSuperiorTexto;
          ...
          instrucoesPixArray.forEach((instrucao, indice) => {
            pdf
              ...
              .text(
                instrucao,
                args.ajusteX + margemDoSegundoBloco,
                args.ajusteY + textoPixY + indice * espacamentoEntreLinhas,
                {
                  width: larguraInstrucoes,
                  align: 'right',
                  lineBreak: false,
