...
const TAMANHO_MINIMO_DA_FONTE = 6;

function ajustarTextoParaLargura(pdf, texto, fonte, larguraMaxima, tamanhoInicial) {
  let tamanhoDaFonte = tamanhoInicial;

  pdf.font(fonte);

  while (
    tamanhoDaFonte > TAMANHO_MINIMO_DA_FONTE &&
    pdf.fontSize(tamanhoDaFonte).widthOfString(texto) > larguraMaxima
  ) {
    tamanhoDaFonte -= 0.5;
  }

  pdf.fontSize(tamanhoDaFonte);

  let textoAjustado = texto;

  if (pdf.widthOfString(textoAjustado) > larguraMaxima) {
    while (textoAjustado.length > 1 && pdf.widthOfString(textoAjustado + '...') > larguraMaxima) {
      textoAjustado = textoAjustado.slice(0, -1);
    }

    textoAjustado = textoAjustado.trimEnd() + '...';
  }
...
class GeradorDeBoleto {
  ...
  async gerarPDF(args) {
    ...
    for (let indice = 0; indice < boletos.length; indice++) {
      ...
      const identificacaoDoPagador = pagador.getIdentificacao(),
        larguraMaximaDaIdentificacao = 294,
        identificacaoAjustada = ajustarTextoParaLargura(
          pdf,
          identificacaoDoPagador,
          'normal',
          larguraMaximaDaIdentificacao,
          args.tamanhoDaFonte
        );

      pdf
        .font('normal')
        .fontSize(identificacaoAjustada.tamanhoDaFonte)
        .text(identificacaoAjustada.texto, args.ajusteX + 32, args.ajusteY + primeiraLinha, {
          lineBreak: false,
          width: larguraMaximaDaIdentificacao,
          align: 'left',
        });
      ...
      boleto.getLocaisDePagamento().forEach(function (localDePagamento, indice, array) {
        if (indice > 1) {
          return;
        }
        ...
        pdf
          .font('normal')
          .fontSize(array.length === 1 ? args.tamanhoDaFonte : args.tamanhoDaFonteDoTitulo)
          .text(
            localDePagamento,
            ...
            args.ajusteY +
              (array.length === 1
                ? terceiraLinha
                : terceiraLinha + 2 - args.tamanhoDaFonte + indice * args.tamanhoDaFonte),
            {
              lineBreak: false,
              width: 400,
              align: 'left',
      ...
      // Calcular largura disponível para instruções quando há QR Code
      const pixEmv = boleto.getPixEmv();
      const larguraInstrucoes = pixEmv ? 240 : 400; // Ajustado para dar espaço à imagem PIX
      ...
      boleto.getInstrucoes().forEach(function (instrucao, indice) {
        pdf
          ...
          .text(
            ...
            {
              lineBreak: false,
              width: larguraInstrucoes,
              align: 'left',
            }
          );
      });

      // Renderizar QR Code PIX no lado direito da área de instruções
      if (pixEmv) {
        const pixRenderer = new PixRenderer(pdf, boleto, args, {
          margemDoSegundoBloco,
          larguraInstrucoes,
          tituloDaSetimaLinha,
        });
