...
const PixRenderer = require('../generators/pdf/layout/pix-renderer');
...
const pdfDefaults = {
  ...
  informacoesPersonalizadas: function (_pdf, _x, _y) {},
};
...
class GeradorDeBoleto {
  ...
  async gerarPDF(args) {
    ...
    for (let indice = 0; indice < boletos.length; indice++) {
      const boleto = boletos[indice];
      // DESENHAR LAYOUT
      ...
      const linha4Opcional = linha3 + ESPACO_ENTRE_LINHAS;
      ...
      pdf
        ...
        .lineTo(
          ...
          args.ajusteY + (banco.exibirReciboDoPagadorCompleto() ? linha3 : linha2)
        )
        .stroke(args.corDoLayout);
      ...
      pdf
        ...
        .lineTo(
          ...
          args.ajusteY + (banco.exibirReciboDoPagadorCompleto() ? linha3 : linha2) + 0.5
        )
        .stroke(args.corDoLayout);
      ...
      pdf
        .moveTo(
          ...
          args.ajusteY + (banco.exibirReciboDoPagadorCompleto() ? linha3 : linha2) + 3.5
        )
        .lineTo(
          args.ajusteX + coluna6,
          args.ajusteY + (banco.exibirReciboDoPagadorCompleto() ? linha4Opcional : linha3) + 0.5
      ...
      // Renderizar QR Code PIX após o código de barras
      const pixEmv = boleto.getPixEmv();
      if (pixEmv) {
        const qrCodeY = linha212 + args.tamanhoDoCodigoDeBarras + 50;
        const pixRenderer = new PixRenderer(pdf, boleto, args, {
          margemDoSegundoBloco: margemDoSegundoBlocoLayout,
          larguraInstrucoes: 0,
          tituloDaSetimaLinha: qrCodeY,
        });
        await pixRenderer.render();

        // Renderizar Pix Copia e Cola ao lado direito do QR Code
        const pixEmvString = boleto.getPixEmvString ? boleto.getPixEmvString() : null;
        if (pixEmvString) {
          // Calcular a mesma posição X das instruções PIX
          const linhaVerticalX = args.ajusteX + margemDoSegundoBlocoLayout - 40;
          const textoPixX = linhaVerticalX + 20; // Mesma posição das instruções

          const pixCopiaColarX = textoPixX;
          const pixCopiaColarY = args.ajusteY + qrCodeY + 105;
          const larguraRetangulo = 575;
          const alturaRetangulo = 50;

          // Desenhar retângulo
          pdf
            .rect(pixCopiaColarX, pixCopiaColarY, larguraRetangulo, alturaRetangulo)
            .stroke(args.corDoLayout);

          // Título "Pix Copia e Cola"
          pdf
            .font('negrito')
            .fontSize(args.tamanhoDaFonteDoTitulo)
            .text('Pix Copia e Cola', pixCopiaColarX + 3, pixCopiaColarY + 3, {
              lineBreak: false,
              width: larguraRetangulo - 6,
              align: 'left',
            });
          ...
          pdf
            .font('normal')
            .fontSize(6)
            .text(pixEmvString, pixCopiaColarX + 3, pixCopiaColarY + 14, {
              lineBreak: true,
              width: larguraRetangulo - 6,
              align: 'left',
            });
      ...
      pdf
        ...
        .text(
          ...
          args.ajusteY + terceiraLinha + 2 - args.tamanhoDaFonte,
          {
            lineBreak: false,
            width: 400,
            align: 'left',
      ...
      if (enderecoDoPagador) {
        let espacamento = args.tamanhoDaFonte;
  ...
  async gerarPNG(args = {}) {
    ...
    const pdfStream = await this.gerarPDF({
      ...options,
      stream: false,
    });

    // Coletar os chunks do PDF
    ...
    // Converter PDF para PNG usando pdf-to-img
    const document = await pdfToImg(pdfBuffer, { scale: options.scale });
  ...
  async gerarHTML(args) {
    if (typeof args === 'function') {
      args = {};
    }
    args = args || {};

    const GeradorDeHTML = require('./html-generator');
    const geradorHTML = new GeradorDeHTML(this._boletos, args);
    return await geradorHTML.gerar();
  }

  async gerarPDFFromHTML(args = {}) {
    if (typeof args === 'function') {
      args = {};
    }

    const HTMLtoPDFConverter = require('./html-to-pdf-converter');
    const html = await this.gerarHTML(args);

    // Extrair opções específicas do Puppeteer
    const puppeteerOptions = {
      format: args.format || 'A4',
      landscape: args.landscape || false,
      margin: args.margin || { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' },
      printBackground: args.printBackground !== undefined ? args.printBackground : true,
    };

    // Se args.stream for especificado, pipe o resultado
    if (args.stream) {
      const pdfBuffer = await HTMLtoPDFConverter.convert(html, puppeteerOptions);
      args.stream.write(pdfBuffer);
      args.stream.end();
      return args.stream;
    }

    // Se outputPath for especificado, salvar em arquivo
    if (args.outputPath) {
      await HTMLtoPDFConverter.convertToFile(html, args.outputPath, puppeteerOptions);
      return args.outputPath;
    }

    // Caso contrário, retornar buffer
    return await HTMLtoPDFConverter.convert(html, puppeteerOptions);
  }
}
