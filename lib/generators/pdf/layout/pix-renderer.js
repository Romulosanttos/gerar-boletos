...
class PixRenderer {
  ...
  calcularPosicoes() {
    ...
    const qrCodeX = this.args.ajusteX + this.margemDoSegundoBloco + this.larguraInstrucoes;
    const qrCodeY = this.args.ajusteY + this.tituloDaSetimaLinha;
  ...
  renderizarLogoPix(posicoes) {
    try {
      const caminhoImagemPix = path.join(__dirname, '../../imagens/pix-logo.png');
