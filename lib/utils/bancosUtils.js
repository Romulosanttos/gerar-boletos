...
function calcularDVDoBradescoOuDoBancoDoBrasil(agenciaOuConta) {
  const size = agenciaOuConta.length;
  let result = 0,
    mult = 2;

  for (let i = size - 1; i >= 0; i--) {
    const temp = agenciaOuConta.charAt(i) * mult;
...
module.exports.itau = {
  validarAgenciaEConta: function (agencia, conta) {
    ...
    const digitoInformado = conta.substr(conta.length - 1),
...
function validarAgenciaOuContaDoBradescoOuBancoDoBrasil(agenciaOuConta) {
  ...
  const digitoInformado = agenciaOuConta.substr(agenciaOuConta.length - 1),
    digitoCalculado = calcularDVDoBradescoOuDoBancoDoBrasil(
      agenciaOuConta.substr(0, agenciaOuConta.length - 1)
    );
...
module.exports.bancoDoBrasil = {
  validarAgenciaEConta: function (agencia, conta) {
    return (
      validarAgenciaOuContaDoBradescoOuBancoDoBrasil(agencia) &&
