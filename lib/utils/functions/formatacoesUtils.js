...
function dinheiroPorExtenso(numero) {
  ...
  let a, v, i;
  const numero_split = numero.replace(c ? /[^,\d]/g : /\D/g, '').split(',');
  const e = ' e ';
  const $ = 'real';
  const d = 'centavo';
  const r = [];

  for (let f = numero_split.length - 1, l, j = -1, s = [], t = ''; ++j <= f; s = []) {
    j && (numero_split[j] = (('.' + numero_split[j]) * 1).toFixed(2).slice(2));
    if (
      !((a = (v = numero_split[j]).slice((l = v.length) % 3).match(/\d{3}/g)),
      (v = l % 3 ? [v.slice(0, l % 3)] : []),
    ...
    for (a = -1, l = v.length; ++a < l; t = '') {
      ...
      (i % 100 < 20 && (t += ex[0][i % 100])) ||
        ((i % 100) + 1 &&
          (t += ex[1][(((i % 100) / 10) >> 0) - 1] + (i % 10 ? e + ex[0][i % 10] : '')));
      s.push(
        (i < 100
          ? t
          : !(i % 100)
            ...
            : ex[2][(i / 100) >> 0] + e + t) +
          ((t = l - a - 2) > -1
            ? ' ' + (i > 1 && t > 0 ? ex[3][t].replace('?o', '?es') : ex[3][t])
            : '')
      );
    }
    a =
      s.length > 1
        ? ((a = s.pop()), s.join(' ') + e + a)
        : s.join('') || ((!j && numero_split[j + 1] * 1 > 0) || r.length ? '' : ex[0][0]);
    a &&
      r.push(
        a +
          (c
            ? ' ' +
              (v.join('') * 1 > 1
                ? j
                  ? d + 's'
                  : (/0{6,}$/.test(numero_split[0]) ? 'de ' : '') + $.replace('l', 'is')
                : j
                  ? d
                  : $)
            : '')
...
function numero(numero, args) {
  ...
  let casasDecimaisOriginais = numero.toString().split('.');
  ...
  function _format(n, x, s, c) {
    ...
    let num = numero
      .toString()
      .replace(new RegExp('\\.([\\d]{' + args.casasDecimais + '})[\\d]*?$'), '.$1');
...
function dataEHora(_data, args) {
  ...
  return [data(_data), hora(_data, args)].join(' ');
}
...
function registroNacional(texto) {
  const tipo = validacoes.eRegistroNacional(texto);
