...
function dinheiroPorExtenso(numero) {
  ...
  var a, v, i, numero_split = numero.replace(c ? /[^,\d]/g : /\D/g, '').split(','), e = ' e ', $ = 'real', d = 'centavo';

  for(var f = numero_split.length - 1, l, j = -1, r = [], s = [], t = ''; ++j <= f; s = []){
    j && (numero_split[j] = (('.' + numero_split[j]) * 1).toFixed(2).slice(2));
    if(!(a = (v = numero_split[j]).slice((l = v.length) % 3).match(/\d{3}/g), v = l % 3 ? [v.slice(0, l % 3)] : [], v = a ? v.concat(a) : v).length) continue;
    ...
    a = (s.length > 1 ? (a = s.pop(), s.join(' ') + e + a) : s.join('') || ((!j && (numero_split[j + 1] * 1 > 0) || r.length) ? '' : ex[0][0]));
    a && r.push(a + (c ? (' ' + (v.join('') * 1 > 1 ? j ? d + 's' : (/0{6,}$/.test(numero_split[0]) ? 'de ' : '') + $.replace('l', 'is') : j ? d : $)) : ''));
  }
